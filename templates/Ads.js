...
class App extends React.Component {
  constructor(props) {super(props); let app = this; window.app = this;
    this.state = {menuHeight: 108, match: -1, swap: -1, swapLocation: '', swapQ: '', swapCategories: [['مغازه و دفتر', 'rent-office', 'rent-commercial-property'], ['ویلا', 'buy-villa', 'buy-residential'], ['آپارتمان', 'buy-apartment', 'buy-residential'], ['اجاره ویلا', 'rent-villa', 'rent-residential'], ['اجاره آپارتمان', 'rent-apartment', 'rent-residential'], ['باغ و کلنگی', 'buy-old-house', 'buy-residential'], ['باغ و زمین', 'industrial-agricultural-property', 'buy-residential'], ['دلار،طلا', 'gold']], swapBudget: '', swapArea: '', swapCapacity: '', swapLiquidity: '', swapDebt: '', swapFill: [], 
      note: -1, noteInput: '', ads: [], show: {}, phone: cookie('phone'), session: cookie('session'), keys: {}, potent: false, potentOtp: '', otp: false, potentPhone: '', potentInterest: '', plyr: true, searchInput: '', noteInput: '', filter: {}, page: 1, searchExpand: false, footExpand: false, rows: 5, trans: true, background: 0, leftMenu: 'لیست سرچ', urlList: '', category: -1, categoryShow: false, firstMenuShow: false, firstMenuIndex: 0, secondMenuShow: false, secondMenuIndex: 0, firstMenuList: [
    ...
    let r =  await fetch(`/ads/${this.state.ads[this.state.swap].id}/swap`, { method: 'POST', body: JSON.stringify(body),
      headers: {'Accept': 'application/json','Content-Type': 'application/json', 'Authorization': this.state.session}});
    ...
    this.state.swapDebt.replace(/^\D+/g, '') && this.state.swapQ.trim() &&
    this.state.swapFill.length == 1 && this.state.swapCategories.length == 1)
  } eraseSwap() { this.setState({swapLocation: '', swapQ: '', swapBudget: '', swapLiquidity: '', swapDebt: '', swapArea: '', swapCapacity: '', 
    swapCategories: [['مغازه و دفتر', 'rent-office', 'rent-commercial-property'], ['ویلا', 'buy-villa', 'buy-residential'], ['آپارتمان', 'buy-apartment', 'buy-residential'], ['اجاره ویلا', 'rent-villa', 'rent-residential'], ['اجاره آپارتمان', 'rent-apartment', 'rent-residential'], ['باغ و کلنگی', 'buy-old-house', 'buy-residential'], ['باغ و زمین', 'industrial-agricultural-property', 'buy-residential'], ['دلار،طلا', 'gold']]});
    ...
    if ('ids' in params) {
      let r = await fetch(`/ads/${params.ids}`);
    ...
    return <div>
      {this.state.potent ? ({/* #macro modules/potent */}) : (<>
        ...
        {this.state.note >= 0 && <div class="ads-dialogue_box" id="notes" style={{fontSize: '1.3em', position: 'fixed', bottom: 1, height: 222, overflowY: 'scroll', background: '#ffff', borderRadius: 8, border: '1px solid grey', zIndex: 898}}>
          ...
          <div class="touchable" style={{fontSize: '1.3em', position: 'absolute', left: 67, bottom: 3, border: '1px solid #ea456e', padding: 3, paddingBottom: 0, paddingTop: 2, borderRadius: 6}} onClick={() => this.setState({swap: this.state.note, note: -1, swapCategories: [['مغازه و دفتر', 'rent-office', 'rent-commercial-property'], ['ویلا', 'buy-villa', 'buy-residential'], ['آپارتمان', 'buy-apartment', 'buy-residential'], ['اجاره ویلا', 'rent-villa', 'rent-residential'], ['اجاره آپارتمان', 'rent-apartment', 'rent-residential'], ['باغ و کلنگی', 'buy-old-house', 'buy-residential'], ['باغ و زمین', 'industrial-agricultural-property', 'buy-residential'], ['دلار،طلا', 'gold']]})}>🔀</div>
          <a class="touchable" style={{fontSize: '1.34em', position: 'absolute', left: 101, bottom: 3, border: '1px solid #ea456e', padding: 2, paddingBottom: 0, paddingTop: 1, borderRadius: 6}} href={`/ads/${this.state.ads[this.state.note].id}/~`} target="_blank" rel="noopener noreferrer">💲</a>
          <div class="touchable" style={{fontSize: '1.25em', position: 'absolute', left: 134, bottom: 3, border: '1px solid #ea456e', padding: 3, paddingBottom: 0, paddingTop: 3, borderRadius: 6}} onClick={async () => {let r = await fetch(`/ads/${this.state.ads[this.state.note].id}/~`, {method: 'POST', body: JSON.stringify({disable: ('disable' in this.state.ads[this.state.note]) ? ! this.state.ads[this.state.note].disable : true})}); if (r.status == 200) {this.state.ads[this.state.note].disable = ('disable' in this.state.ads[this.state.note]) ? ! this.state.ads[this.state.note].disable : true; this.setState({note: this.state.note})}}}>❌</div>
        </div>} {this.state.swap >= 0 && <div class="ads-dialogue_box" id="swap" style={{marginBottom: 0, fontSize: '1.3em', position: 'fixed', bottom: 1, height: 222, overflow: 'hidden', background: '#ffff', borderRadius: 8, border: '1px solid grey', zIndex: 899}}>
          <div style={{verticalAlign: 'middle', display: 'inline-block', width: '50%', height: '100%', paddingTop: 2}}>
            {this.state.swapCategories.map((t, i_t) => <span class="touchable" style={{borderRadius: 14, backgroundColor: '#ddd', padding: 2, paddingLeft: 8, paddingRight: 8, marginRight: 4, lineHeight: '2.2em'}} onClick={() => {this.setState({swapCategories: [t]})}}>{t[0]}</span>)}
            <input autoFocus placeholder="مشخصات" id="swap-q" style={{position: 'absolute', bottom: 0, width: 'calc(50% - 18px)', border: 'none', outline: 'none', borderTop: '1px grey solid', lineHeight: '3em', fontSize: '.95em', right: 9}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}
              onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} onChange={async (e) => {this.setState({swapQ: e.target.value})}}/>
          </div>
          <div style={{verticalAlign: 'middle', display: 'inline-block', width: '50%', height: '100%'}}>
            {this.state.swapFill.map((fill, i_fill) => <div class="touchable" style={{marginTop: i_fill == 0 ? 0 : 8}} onClick={() => this.setState({swapFill: [fill], swapLocation: fill.keywords.join('، ')})}>{fill.keywords.join('، ')}</div>)}
            <input placeholder="مکان" style={{position: 'absolute', bottom: 0, width: 'calc(50% - 18px)', border: 'none', outline: 'none', borderTop: '1px grey solid', lineHeight: '3em', fontSize: '.95em', left: 9}} autocomplete="off"  onChange={async (e) => {let loc = e.target.value.trim(); this.setState({swapLocation: loc}); if (loc.length < 2) return; let r = await fetch(`/fill/${loc}`); this.setState({swapFill: (await r.json()).polygons})}}
              onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} value={this.state.swapLocation} onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}/>
          </div>
          <input value={this.state.swapBudget} placeholder="بودجه میلیون" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', right: 5, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} onChange={async (e) => {this.setState({swapBudget: e.target.value})}}/>
          <input value={this.state.swapLiquidity} placeholder="دلار،طلا" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', right: 109, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} onChange={async (e) => {this.setState({swapLiquidity: e.target.value})}}/>
          <input value={this.state.swapArea} placeholder="متراژ،تعداد" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 'calc(50% + 2px)', textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} onChange={async (e) => {this.setState({swapArea: e.target.value})}}/>
          <input value={this.state.swapCapacity} placeholder="متراژ،تعداد" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 'calc(50% - 102px)', textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter' && this.swapable()) await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') {this.eraseSwap(); this.setState({swap: -1})}}} onChange={async (e) => {this.setState({swapCapacity: e.target.value})}}/>
        ...
        </div>} {this.state.match >= 0 && <div class="ads-dialogue_box" id="matches" style={{overflowX: 'hidden', fontSize: '1.3em', position: 'fixed', bottom: 1, height: 222, overflowY: 'scroll', background: '#ffff', borderRadius: 8, border: '1px solid grey', zIndex: 798}}>
          ...
          {this.state.ads.map((ad, adi) => <div class="touchable" style={{paddingTop: 10, paddingBottom: 5}} onClick={() => {console.log(ad.id); this.setState({show: ad})}}>
            ...
            <div class="narrow_fit_text" style={{textAlign: 'justify', display: 'inline-block', verticalAlign: 'top', position: 'relative'}}>
              ...
              <div class="narrow_sub" style={{width: '100%'}}>
                ...
                <a style={{float: 'right', textDecoration: 'none', color: '#343747', fontWeight: 500, cursor: 'pointer', marginRight: 10, marginTop: window.innerWidth > 768 ? 0 : 2}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); window.open(`tel:${ad.phone}`, '_self')}}>{ad.phone}</a>
                ...
                <a style={{float: 'left', textDecoration: 'none', color: '#f43747', fontSize: '.85em', fontWeight: 500, cursor: 'pointer', paddingRight: 10}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); window.scrollTo({ top: window.scrollY + e.target.getBoundingClientRect().top - 342 }); this.setState({match: -1, note: adi})}}>یادداشت<span style={{backgroundColor: '#f43747', color: 'white', borderRadius: 10, width: 20, height: 20, display: 'inline-block', textAlign: 'center', verticalAlign: 'middle', marginRight: 5, lineHeight: '20px'}}>{ad.notes.length.farsify()}</span></a>
                { (ad.swap || ad.matches) && <a style={{float: 'left', textDecoration: 'none', color: '#f43747', fontSize: '.85em', fontWeight: 500, cursor: 'pointer', marginTop: window.innerWidth > 768 ? -6 : -4}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); window.scrollTo({ top: window.scrollY + e.target.getBoundingClientRect().top - 342 }); this.setState({match: adi, note: -1})}}><span style={{backgroundColor: '#f43747', color: 'white', borderRadius: 10, width: 20, height: 20, display: 'inline-block', textAlign: 'center', verticalAlign: 'middle', marginRight: 5, lineHeight: '20px'}}>{('matches' in ad) ? ad.matches.length.farsify() : '۰'}</span><span style={{fontSize: '1.6em', verticalAlign: 'middle'}}>🤝</span></a>}
                { (window.innerWidth > 768 && ad.disable) && <span style={{float: 'left', textDecoration: 'none', color: '#e1a400', fontWeight: 700, fontSize: '.8em', paddingLeft: 8}}>❌</span> }
                { (window.innerWidth > 768 && ad.new) && <span style={{float: 'left', textDecoration: 'none', color: '#e1a400', fontWeight: 700, fontSize: '.8em', paddingLeft: 8}}>new</span>}
              </div>
            </div>
          </div>)}
        </div>
