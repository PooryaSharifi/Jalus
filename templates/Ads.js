...
class App extends React.Component {
  constructor(props) {super(props); let app = this; window.app = this;
    this.state = {swap: -1, swapLocation: '', swapQ: '', swapCategories: [['مغازه و دفتر', 'rent-office'], ['ویلا', 'buy-villa'], ['آپارتمان', 'buy-apartment'], ['اجاره ویلا', 'rent-villa'], ['اجاره آپارتمان', 'rent-apartment'], ['باغ و کلنگی', 'buy-old-house'], ['باغ و زمین', 'industrial-agricultural-property']], swapBudget: '', swapArea: '', swapCapacity: '', swapFill: [], note: -1, noteInput: '', ads: [], show: {}, phone: cookie('phone'), session: cookie('session'), keys: {}, potent: false, potentOtp: '', otp: false, potentPhone: '', potentInterest: '', plyr: true, searchInput: '', noteInput: '', filter: {}, page: 1, searchExpand: false, footExpand: false, rows: 5, trans: true, background: 0, leftMenu: 'لیست سرچ', urlList: '', category: -1, categoryShow: false, firstMenuShow: false, firstMenuIndex: 0, secondMenuShow: false, secondMenuIndex: 0, firstMenuList: [
    ...
    console.log(this.state.swapArea)
      ...
      {this.state.potent ? ({/* #macro modules/potent */}) : (<>
        ...
        {this.state.note >= 0 && <div id="notes" style={{fontSize: '1.3em', position: 'fixed', bottom: 1, left: 80, right: 80, height: 222, overflowY: 'scroll', background: '#ffff', borderRadius: 8, border: '1px solid grey', zIndex: 898}}>
          ...
          <div style={{verticalAlign: 'middle', display: 'inline-block', width: '50%', height: '100%', paddingTop: 2}}>
            ...
            <input autoFocus placeholder="مشخصات" id="swap-q" style={{position: 'absolute', bottom: 0, width: 'calc(50% - 18px)', border: 'none', outline: 'none', borderTop: '1px grey solid', lineHeight: '3em', fontSize: '.95em', right: 9}} autocomplete="off" onKeyUp={() => {console.log('lew')}}/>
          </div>
          <div style={{verticalAlign: 'middle', display: 'inline-block', width: '50%', height: '100%'}}>
            {this.state.swapFill.map((fill, i_fill) => <div class="touchable" style={{marginTop: i_fill == 0 ? 0 : 8}} onClick={() => this.setState({swapFill: [fill]})}>{fill.keywords.join('، ')}</div>)}
            <input placeholder="مکان" style={{position: 'absolute', bottom: 0, width: 'calc(50% - 18px)', border: 'none', outline: 'none', borderTop: '1px grey solid', lineHeight: '3em', fontSize: '.95em', left: 9}} autocomplete="off"  onChange={async (e) => {let loc = e.target.value.trim(); this.setState({swapLocation: loc}); if (loc.length < 2) return; let r = await fetch(`/fill/${loc}`); this.setState({swapFill: (await r.json()).polygons})}}
              onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1});}} value={this.state.swapLocation} onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}/>
          </div>
          <input value={this.state.swapBudget} placeholder="بودجه میلیون" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', right: 5, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapBudget: e.target.value})}}/>
          <input value={this.state.swapLiquidity} placeholder="دلار،طلا" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', right: 109, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapBudget: e.target.value})}}/>
          <input value={this.state.swapArea} placeholder="متراژ،تعداد" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 'calc(50% + 2px)', textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapArea: e.target.value})}}/>
          <input value={this.state.swapCapacity} placeholder="متراژ،تعداد" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 'calc(50% - 102px)', textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapCapacity: e.target.value})}}/>
          <input value={this.state.swapLiquidity} placeholder="وام" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 109, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapBudget: e.target.value})}}/>
          <input value={this.state.swapLiquidity} placeholder="اعمال" style={{position: 'absolute', bottom: 41, width: 100, border: '1px grey solid', outline: 'none', borderRadius: 40, lineHeight: '3em', fontSize: '.95em', display: 'inline-block', left: 5, textAlign: 'center', backgroundColor: 'white'}} autocomplete="off" onKeyPress={async (e) => {if(e.key === 'Enter') await this.swap()}}
            onBlur={(e) => {let target = e.nativeEvent.explicitOriginalTarget; if(target.id != 'swap' && target.parentElement.id != 'swap' && target.parentElement.parentElement.id != 'swap' && target.parentElement.parentElement.parentElement.id != 'swap') this.setState({swap: -1})}} onChange={async (e) => {this.setState({swapBudget: e.target.value})}}/>
