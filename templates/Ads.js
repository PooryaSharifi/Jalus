          ...
          {this.state.ads.map((ad, adi) => <div class="touchable" style={{paddingTop: 10, paddingBottom: 5}} onClick={() => {console.log(ad.id); this.setState({show: ad})}}>
            ...
            <div class="narrow_fit_text" style={{textAlign: 'justify', display: 'inline-block', verticalAlign: 'top', position: 'relative'}}>
              ...
              <div class="narrow_sub" style={{width: '100%'}}>
                ...
                { (ad.swap || ad.matches) && <a style={{float: 'left', textDecoration: 'none', color: '#f43747', fontSize: '.85em', fontWeight: 500, cursor: 'pointer', paddingRight: 10, marginTop: -6}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); window.scrollTo({ top: window.scrollY + e.target.getBoundingClientRect().top - 342 }); this.setState({match: adi, note: -1})}}><span style={{backgroundColor: '#f43747', color: 'white', borderRadius: 10, width: 20, height: 20, display: 'inline-block', textAlign: 'center', verticalAlign: 'middle', marginRight: 5, lineHeight: '20px'}}>{('matches' in ad) ? ad.matches.length.farsify() : '۰'}</span><span style={{fontSize: '1.6em', verticalAlign: 'middle'}}>🤝</span></a>}
                { ad.disable && <span style={{float: 'left', textDecoration: 'none', color: '#e1a400', fontWeight: 700, fontSize: '.8em'}}>❌</span> }
                { ad.new && <span style={{float: 'left', textDecoration: 'none', color: '#e1a400', fontWeight: 700, fontSize: '.8em'}}>new</span>}
              </div>
            </div>
          </div>)}
        </div>
