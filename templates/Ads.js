          ...
          {this.state.note >= 0 && <div id="notes" style={{fontSize: '1.3em', position: 'fixed', bottom: 1, left: 80, right: 80, height: 222, overflowY: 'scroll', background: '#ffff', borderRadius: 8, border: '1px solid grey', zIndex: 899}}>
            ...
            {this.state.ads.map((ad, adi) => <div class="touchable" style={{paddingTop: 10, paddingBottom: 5}} onClick={() => {console.log(ad.id); this.setState({show: ad})}}>
              ...
              <div class="narrow_fit_text" style={{textAlign: 'justify', display: 'inline-block', verticalAlign: 'top', minHeight: 254, position: 'relative'}}>
                ...
                <div style={{position: 'absolute', bottom: -3.5, textDecoration: 'none', color: '#343747', fontWeight: 500, cursor: 'pointer'}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); console.log('hu ha ha')}}><div style={{backgroundImage: `url(/static/consultants/${ad.consultant.icon})`, backgroundPosition: 'center center', backgroundRepeat: 'no-repeat', backgroundSize: 'contain', height: 30, width: 30, display: 'inline-block', verticalAlign: 'middle', marginLeft: 4}}/>{ad.consultant.name}</div>
                <a style={{position: 'absolute', bottom: 0, textDecoration: 'none', color: '#343747', fontWeight: 500, cursor: 'pointer', right: ad.consultant.name.length * 8 + 55}} onClick={(e) => {e.preventDefault(); e.stopPropagation(); e.nativeEvent.stopImmediatePropagation(); window.open(`tel:${ad.phone}`, '_self')}}>{ad.phone}</a>
