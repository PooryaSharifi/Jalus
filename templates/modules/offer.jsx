<div class="container-2xl-w mx-auto lg:px-4 2xl:px-0 px-5">
  <div>
    <a class="relative overflow-hidden bg-neutral-100 user-select-none w-full flex flex-col lg:flex-row justify-between rounded-large lg:items-center px-5 lg:px-10 pt-4 pb-3 styles_AmazingShortcut__main__EbBRH" target="_blank" href={this.state.page == 'strategist' ? '/strategist/deev' : `/users?offer=1`}>
      ...
      <div class="flex flex-col lg:flex-row items-start lg:items-center z-1 shrink-0 gap-2 lg:gap-0">
        <div class="flex items-center z-1 shrink-0">
          ...
          <div class="mx-2 lg:mx-5" style={{width: 250, height: 28, lineHeight: 0}}><img
              class="w-full inline-block" src={this.state.page == 'strategist' ? "/static/icon/strategist-incredibles.svg" : "/static/icon/fresh-incredible-offer.svg"} width="250"
      ...
      <div class="flex items-center justify-between lg:justify-start mt-4 lg:mt-0 lg:mr-4 z-1">
        <div class="flex items-center lg:justify-end flex-nowrap lg:flex-wrap overflow-hidden styles_AmazingShortcut__productItems__B6LWb" style={{paddingRight: this.state.page == 'strategist' ? 10 : 0}}>
          {this.state.offers.map((offer) => (
            <a class="relative bg-neutral-000 rounded-circle flex items-center justify-center p-1.5 ml-2 lg:mb-4" href={this.state.page == 'strategist' ? '/strategist/deev' : `/users?ids=${offer.id}`}>
              <div class="overflow-hidden rounded-circle">
                <div style={{width: 64, height: 64, textAlign: 'center', alignContent: 'center'}}>
                  {this.state.page == 'strategist' ? <>{offer.name}<br/>{offer.achievement}</> : <picture>
              ...
              {this.state.page == 'strategist' ? <div style={{right: -10, height: 20}} class="px-1 text-white rounded-large flex items-center justify-center bg-hint-object-error absolute bottom-0 z-1">
                <span class="text-body2-strong" data-testid="price-discount-percent">رتبه{offer.quota.farsify()}</span>
              </div> : <div class="px-1 text-white rounded-large flex items-center justify-center ProductPrice_ProductPrice__discountWrapper__1Ru_1 bg-hint-object-error absolute right-0 bottom-0 z-1">
        ...
        <div class="bg-neutral-000 p-3 lg:py-3 lg:px-5 text-button-2 flex items-center justify-center text-fresh-700 styles_AmazingShortcut__button__DU7l4">
          <span class="hidden lg:inline whitespace-nowrap cursor-pointer">{this.state.page == 'strategist' ? 'نتایج دیوسالاران' : 'بیش از ۵۰ ویلا'}</span>
