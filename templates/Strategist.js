...
class App extends React.Component {
  constructor(props) {super(props); let app = this; window.app = this; window.mode_11 = ['معدل کل', 'تراز کل', 'ریز نمره']; window.mode_12 = ['معدل کل', 'تراز کل', 'ریز نمره']; window.mode_13 = ['درصد کلی', 'تراز کنکور', 'درصد دروس']; window.field = [['empirical', 'تجربی'], ['technical', 'ریاضی'], ['humanity', 'انسانی']]; window.quota = [[1, 'منطقه یک'], [2, 'منطقه دو'], [3, 'منطقه سه'], [5, 'سهمیه ۵ درصد'], [15, 'سهمیه ۱۵ درصد']]
    ...
    predict_result: {}, predict: {
      ...
      physics_technical_12: '', physics_empirical_12: '', mathematics_all_12: '', geometry_technical_12: '', chemistry_technical_12: '', chemistry_empirical_12: '', arabic_all_12: '', religion_all_12: '', english_all_12: '', litrature_all_12: '', social_all_12: '', health_all_12: '', discrete_technical_12: '', philosophy_humanity_12: '', history_humanity_12: '', geography_humanity_12: '', technology_humanity_12: '', biology_empirical_12: '',
      mathematics_all_13: '', physics_empirical_13: '', chemistry_empirical_13: '', biology_empirical_13: '', geology_empirical_13: '', chemistry_technical_13: '', physics_technical_13: '', finance_humanity_13: '', litrature_humanity_13: '', arabic_humanity_13: '', history_humanity_13: '', social_humanity_13: '', psychology_humanity_13: '', logic_humanity_13: '',
          ...
          <div style={{background: 'linear-gradient(transparent 0%, white 100%), linear-gradient(transparent 0%, white 100%), linear-gradient(transparent 0%, white 100%), linear-gradient(to left, #654EA3 0%, #EAAFC8 100%)', maxWidth: 768, minWidth: Math.min(480, window.innerWidth), padding: 40, paddingTop: 25, borderRadius: 16, marginTop: 10}}>
            ...
            {([[13, 'کنکور'], [12, 'دوازدهم'], [11, 'یازدهم']]).map((grade, gi) => <>
              ...
              <div style={{textAlign: 'center'}}>{Object.keys(this.state.predict).filter((key) => {let keys = key.split('_'); return (keys[2] == '' + grade[0] && (keys[1] == 'all' || keys[1] == window.field[this.state.field][0]) && (window[`mode_${grade[0]}`].length - 1 == this.state[`mode_${grade[0]}`] ? keys[0] != 'total' : keys[0] == 'total'))}).map((key) => <div style={{display: 'inline-block', marginLeft: 14, marginRight: 14, marginBottom: 20}}>
                <input value={this.state.predict[key]} onChange={(e) => {this.state.predict[key] = e.target.value.deFarsify(); this.setState({predict: this.state.predict})}} style={{border: gi % 2 == 0 ? '1px solid #896fb1' : '1px solid #24c386', width: window[`mode_${grade[0]}`].length - 1 == this.state[`mode_${grade[0]}`] ? 70 : 140, backgroundColor: '#fffb', paddingTop: 8, paddingBottom: 8, textAlign: 'center'}} class="px-2 TextField_TextField__input__hFMFl text-subtitle w-full TextField_TextField__bwN9_ TextField_TextField--secondary__w_vGF text-subtitle w-full py-5 lg:py-2 rounded-medium" type="text"/>
