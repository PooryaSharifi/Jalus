...
/** #links <link rel="stylesheet" href="/static/map.css"/> #links **/
...
class App extends React.Component {
  constructor(props) {super(props);  // condition: Clear, Cloudy, Rainy, Snowy
    ...
    this.state = {searchExpand: false, searchInput: '', searchLocations: [], searchProperties: [], searchFeatures: [], expand_lyr: false, lyr: 0, landing: landing == 1, 
    ...
    let app = this; window.app = this; this.checkReserved = this.checkReserved.bind(this); this.binSearch = this.binSearch.bind(this); this.calendar = this.calendar.bind(this);
    ...
    return (<div style={{height: '100%'}}>
      <div className="categories" style={{backgroundColor: ['#353331', '#ffffff', '#ffffff'][this.state.lyr], direction: 'ltr', borderTop: `1px solid ${['rgb(47, 39, 49)', '#fff9fa', '#fff9fa'][this.state.lyr]}`, height: '100%'}}>
      ...
      </div> <div style={{opacity: '98%', position: 'fixed', right: 5, top: 5, minHeight: 50, width: 50, borderRadius: 16, zIndex: 798, backgroundColor: '#ffffff', overflow: 'hidden', padding: 3, paddingBottom: 0, cursor: 'pointer', textAlign: 'center'}}>
      ...
      </div> <div style={{opacity: '98%', position: 'fixed', right: 5, top: 60, minHeight: 50, width: 50, borderRadius: 16, zIndex: 798, backgroundColor: '#ffffff', overflow: 'hidden', padding: 3, paddingBottom: 0, cursor: 'pointer', height: this.state.expand_lyr ? 'auto' : 50}}>
        ...
        <div className="scroll" style={{height: '100%', backgroundColor: 'white', borderRadius: 16, direction: 'rtl', overflowY: 'scroll'}}>
          <input class="none" style={{'-webkit-appearance': 'none', outline: 'none', display: 'block', width: '100%', borderRadius: 10, borderWidth: 0, borderColor: 'white', lineHeight: '1.6em', fontSize: '1.3em', padding: '.4em', direction: 'rtl', color: '#343747'}} autoFocus type="text" name="search-input" placeholder="جستجو" autocomplete="off" value={this.state.searchInput} onBlur={() => {this.setState({searchExpand: false})}} onChange={async (e) => {this.setState({searchInput: e.target.value})}} onKeyPress={async (e) => {if(e.key === 'Enter') {
            console.log('SearchInput'); this.setState({searchInput: '', searchExpand: false})
          }}}/>
        </div>
        ...
        <div className="scroll" style={{backgroundColor: 'white', borderRadius: 16, direction: 'rtl', overflowY: 'scroll', padding: 9}}>
          ...
          : <div className="touchable" style={{padding: '13px 14px 15px 14px', color: 'white', backgroundColor: '#e31025', textAlign: 'center', fontWeight: 700, borderRadius: 12, fontSize: '1.02em'}} onClick={async (e) => {let r = await fetch(`/otp/${this.state.phone.trim().deFarsify()}/`); if (r.status < 300) {if (!this.state.phone) return; r = await r.json(); console.log(r); this.setState({otp: true}); }}}>{'ارسال پیامک'}</div>}
        </div>
