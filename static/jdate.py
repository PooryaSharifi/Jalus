...
def gregorian_to_jd(year, month, day):
    ...
    -math.floor((year - 1) / 100)) + math.floor((year - 1) / 400) + math.floor(
        (((367 * month) - 362) / 12) + (0 if (month <= 2) else -1 if leap_gregorian(year) else -2) + day)
...
def persian_to_jd(year, month, day):
    ...
    return day + (((month - 1) * 31) if (month <= 7) else(((month - 1) * 30) + 6)) + math.floor(
        ((epyear * 682) - 110) / 2816) + (epyear - 1) * 365 + math.floor(epbase / 2820) * 1029983 + (PERSIAN_EPOCH - 1)
...
def bahai_to_jd(major, cycle, year, month, day):
    ...
    0 if (month != 20) else  -14 if leap_gregorian(gy + 1) else -15) + day
