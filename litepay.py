...
def push_ads():  # http://192.168.0.55:5000, https://jalus.ir
    ...
    while True:
        ...
        for i_ad, ad in enumerate(new_ads):
            ad['images'] = [im for _i_im, im in enumerate(ad['images']) if os.path.exists(f'{os.path.dirname(os.path.abspath(__file__))}/static/properties/{ad["category"]}/{ad["id"]}/{_i_im}.webp') 
                            and os.path.getsize(f'{os.path.dirname(os.path.abspath(__file__))}/static/properties/{ad["category"]}/{ad["id"]}/{_i_im}.webp') > 3000][:(8 if collection == 0 else 3)]
            ...
            if i_im >= len(ad['images']) or ad['images'][i_im] != 'network':
